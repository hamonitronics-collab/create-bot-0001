...
class SushiswapV3Adapter(BaseV3Adapter):
    """SushiSwap V3用のアダプター (BaseV3Adapterを継承)"""
    def __init__(self, w3: Web3, logger, config: dict):
        ...
        quoter_address = config.get('dexes', {}).get('sushiswap_v3', {}).get('quoter_address')
        if not quoter_address:
            raise ValueError("❌ dexes.yaml に sushiswap_v3 の quoter_address が設定されていません！")
